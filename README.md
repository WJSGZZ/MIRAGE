...
与 **Clash Verge Rev / mihomo** 配合完成系统代理、TUN 全流量接管和规则分流。
...
├─────────────────────────────────┤
│      外层加密（TLS 1.3 / uTLS）  │  唯一加密层，仿真浏览器握手指纹
└─────────────────────────────────┘
               │ TCP
```
...
No standalone GUI is required. MIRAGE exposes standard SOCKS5/HTTP interfaces and a local subscription endpoint, delegating system proxy management, TUN capture, and traffic rules to **Clash Verge Rev / mihomo**.
...
```
┌──────────────────────────────────┐
│      Multiplexing  (Yamux)       │  concurrent stream management
├──────────────────────────────────┤
